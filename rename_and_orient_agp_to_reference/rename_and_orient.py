...
def filter_paf_records(records: List[PAFRecord], 
                       ...
                       target_prefix: str = "chr_") -> List[PAFRecord]:
    """
    ...
    Args:
        ...
        target_prefix: Required prefix for target names
        
    Returns:
        Filtered list of PAF records
    """
    filtered = []
...
def validate_paf_fasta_consistency(
    ...
    """
    Validate consistency between PAF and FASTA files for SUPER_* chromosomes.
    
    Checks that all SUPER_* chromosomes in PAF exist in FASTA and vice versa.
    
    Args:
        paf_records: List of PAF records
        fasta_sequences: Dictionary of FASTA sequences
        query_prefix: Prefix for chromosome names (default "SUPER_")
    ...
    """
    # Get unique SUPER_* names from PAF (excluding unloc)
    paf_chromosomes = set()
    ...
    # Get SUPER_* names from FASTA (excluding unloc)
...
class ChromosomeMapping:
    ...
    def target_suffix(self) -> str:
        """Extract suffix from target name (e.g., 'chr_5' -> '5', 'chr_W' -> 'W')."""
        return self.target_name.replace("chr_", "")
...
def extract_chromosome_suffix(name: str, prefix: str = "SUPER_") -> str:
    ...
    Args:
        name: Chromosome name (e.g., "SUPER_5", "SUPER_W", "chr_Z1")
...
def resolve_chromosome_assignments(
    ...
    all_chr_names = {name for name in sequences.keys() 
                     if name.startswith("SUPER_") and "_unloc_" not in name}
    ...
    for m in mappings:
        query_suffix = extract_chromosome_suffix(m.query_name, "SUPER_")
    ...
    for name in unmapped_names:
        suffix = extract_chromosome_suffix(name, "SUPER_")
    ...
    for m in sex_mappings:
        query_suffix = extract_chromosome_suffix(m.query_name, "SUPER_")
        target_suffix = extract_chromosome_suffix(m.target_name, "chr_")
        ...
        assignment = FinalChromosomeAssignment(
            ...
            new_name=f"SUPER_{query_suffix}",  # Keep original suffix
    ...
    for m in deferred_autosomes:
        ...
        print(f"  Reassigned: {m.query_name} -> SUPER_{next_available}")
        next_available += 1
    ...
    for m in autosomes_to_sex_target:
        ...
        assigned_numbers.add(next_available)
        unavailable.add(next_available)
        print(f"  Reassigned (was sex target): {m.query_name} -> SUPER_{next_available}")
        next_available += 1
    ...
    # Handle unmapped chromosomes - they keep original names
    for name in unmapped_names:
        suffix = extract_chromosome_suffix(name, "SUPER_")
        assignment = FinalChromosomeAssignment(
            original_name=name,
        ...
        print(f"  Unmapped: {name} - keeping original name and orientation")
...
def write_fasta(
    ...
    with open(output_path, 'w') as f:
        ...
        for unloc in unloc_sorted:
            ...
            # Generate new name: SUPER_<new_parent_suffix>_unloc_<N>
            new_parent_suffix = parent_suffix_lookup.get(unloc.parent_chromosome, 
                                                         extract_chromosome_suffix(unloc.parent_chromosome, "SUPER_"))
            new_name = f"SUPER_{new_parent_suffix}_unloc_{unloc.unloc_number}"
...
def write_chromosome_list(
    ...
    Format: SUPER_N,suffix,yes/no (yes for main chr, no for unloc)
...
def main():
    ...
    # Filter records
    filtered_records = filter_paf_records(paf_records)
    print(f"  After filtering (SUPER_* -> chr_*): {len(filtered_records)} records")
    ...
    if in_fasta_not_paf:
        ...
        print(f"           These will keep original names (no alignment data).")
    
    if is_valid:
        print("  OK: All SUPER_* chromosomes match between PAF and FASTA")
    
    # Build chromosome mappings
    print(f"\nBuilding chromosome mappings (min coverage: {args.min_coverage:.0%})...")
    mappings = build_chromosome_mappings(filtered_records, args.min_coverage)
    ...
    assignments, rc_lookup = resolve_chromosome_assignments(mappings, sequences)
    ...
    write_fasta(sequences, sorted_assignments, unloc_mappings, rc_lookup, fasta_output_path)
    write_chromosome_list(sorted_assignments, unloc_mappings, csv_output_path)
