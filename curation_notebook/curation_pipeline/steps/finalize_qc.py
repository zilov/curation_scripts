...
from curation_pipeline.output import console, print_done, print_info, print_step_header, print_warning
from curation_pipeline.steps._helpers import _run
...
def finalize_for_qc(ctx: CurationContext) -> None:
    ...
    remapped_pattern = str(
        ctx.workdir / f"{ctx.tol_id}_curationpretext"
    ...
    if ctx.print_only:
        ...
        pretext_dest_name = (
            f"{tol_id}.{ctx.release_version}.{ctx.hap1_prefix}.curated.pretext"
        )
    else:
