...
def run_rename_and_orient(ctx: CurationContext) -> None:
    ...
    Steps:
        ...
        3. Run rename_and_orient.py script::
               ...
               python3 {script_path} --fasta {fasta} --paf {paf} --output-dir {outdir} --output-prefix {prefix}
    ...
    if ctx.print_only:
        ...
        if not paf_matches:
            raise FileNotFoundError(f"No FastGA PAF found: {paf_pattern}\nRun 'fastga' command first.")
    ...
    script_path = Path(__file__).parent.parent.parent / "rename_and_orient_fasta_to_reference" / "rename_and_orient.py"
