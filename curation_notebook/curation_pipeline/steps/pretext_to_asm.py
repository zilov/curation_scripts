...
from curation_pipeline.output import print_done, print_info, print_next_step, print_step_header
...
def run_pretext_to_asm(ctx: CurationContext) -> None:
    ...
    if not ctx.print_only and not original_fa.exists():
        raise FileNotFoundError(f"original.fa not found at {original_fa}. Run setup_curation first.")
