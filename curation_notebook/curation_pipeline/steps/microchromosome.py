...
def run_microchromosome_curation(ctx: CurationContext) -> None:
    ...
    hap1_fa = hap1_fa_list[0] if hap1_fa_list else str(ctx.workdir / f"{ctx.tol_id}.hap1.primary.curated.fa")
    hap1_chr = hap1_chr_list[0] if hap1_chr_list else str(ctx.workdir / f"{ctx.tol_id}.hap1.primary.chromosome.list.csv")
...
def run_microchromosome_post_curation(ctx: CurationContext) -> None:
    ...
    small_pretext_to_asm = (
        f"module load grit && pretext-to-asm "
        f"-a {merged_small_fa} -p {agp} -o {small_out_fa}"
    )
    ...
    large_hap1_fa_matches = glob.glob(str(micro_dir / f"{ctx.tol_id}*hap1*.primary.curated.large.fa"))
    large_hap1_fa = large_hap1_fa_matches[0] if large_hap1_fa_matches else str(micro_dir / f"{ctx.tol_id}.hap1.primary.curated.large.fa")
    large_hap1_chr_matches = glob.glob(str(micro_dir / f"{ctx.tol_id}*hap1*.large.chr_list.csv"))
    large_hap1_chr = large_hap1_chr_matches[0] if large_hap1_chr_matches else str(micro_dir / f"{ctx.tol_id}.hap1.large.chr_list.csv")

    has_hap2_large = bool(glob.glob(str(micro_dir / f"{ctx.tol_id}*hap2*.primary.curated.large.fa")))
    if ctx.print_only:
    ...
    if has_hap2_large:
        large_hap2_fa_matches = glob.glob(str(micro_dir / f"{ctx.tol_id}*hap2*.primary.curated.large.fa"))
        large_hap2_fa = large_hap2_fa_matches[0] if large_hap2_fa_matches else str(micro_dir / f"{ctx.tol_id}.hap2.primary.curated.large.fa")
        large_hap2_chr_matches = glob.glob(str(micro_dir / f"{ctx.tol_id}*hap2*.large.chr_list.csv"))
        large_hap2_chr = large_hap2_chr_matches[0] if large_hap2_chr_matches else str(micro_dir / f"{ctx.tol_id}.hap2.large.chr_list.csv")
