...
@patch("curation_pipeline.steps.pretext_to_asm._run")
@patch("curation_pipeline.steps.pretext_to_asm.glob.glob")
...
@patch("curation_pipeline.steps.pretext_to_asm._run")
@patch("curation_pipeline.steps.pretext_to_asm.glob.glob", return_value=[])
...
def test_ensure_haplotig_files_creates_empty_for_hap1(mock_ctx, tmp_path):
    mock_ctx.workdir = tmp_path
    mock_ctx.tol_id = "sDipInt39"
...
def test_ensure_haplotig_files_creates_empty_for_primary(mock_ctx_primary, tmp_path):
    ...
    ensure_haplotig_files(mock_ctx_primary)

    expected = tmp_path / "ilHelSara1.1.all_haplotigs.curated.fa"
    assert expected.exists()


def test_ensure_haplotig_files_nonempty_does_not_overwrite(mock_ctx, tmp_path):
...
def test_ensure_haplotig_files_print_only(mock_ctx, tmp_path):
...
@patch("curation_pipeline.steps.hic_remapping._submit_bsub")
@patch("curation_pipeline.steps.hic_remapping.glob.glob")
...
@patch("curation_pipeline.steps.hic_remapping._submit_bsub")
@patch("curation_pipeline.steps.hic_remapping.glob.glob", return_value=[])
...
@patch("curation_pipeline.steps.qv._submit_bsub")
...
@patch("curation_pipeline.steps.finalize_qc._run")
@patch("curation_pipeline.steps.finalize_qc.glob.glob")
