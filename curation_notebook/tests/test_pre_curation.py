...
@patch("curation_pipeline.steps.pre_curation._run")
@patch("curation_pipeline.steps.pre_curation.glob.glob")
def test_setup_curation_initial_hap1hap2(mock_glob, mock_run, mock_ctx, tmp_path):
    ...
    with patch(
        "curation_pipeline.steps.pre_curation._sort_by_mtime", side_effect=lambda x: x
    ):
...
def test_setup_curation_initial_single_hap(mock_glob, mock_run, mock_ctx, tmp_path):
    ...
    zcat_calls = [
        str(c) for c in mock_run.call_args_list if "zcat" in str(c)
    ]
...
@patch("curation_pipeline.steps.pre_curation._run")
@patch("curation_pipeline.steps.pre_curation.glob.glob", return_value=[])
